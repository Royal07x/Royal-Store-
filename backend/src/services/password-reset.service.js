...
export async function issuePasswordReset(user) {
  if (env.RESET_DELIVERY_MODE === 'console' && env.NODE_ENV === 'production') {
    throw new Error('Console password-reset delivery is not allowed in production.');
  }
  if (env.RESET_DELIVERY_MODE !== 'console' && env.RESET_DELIVERY_MODE !== 'email') {
